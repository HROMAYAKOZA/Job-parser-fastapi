...
async def jobs(db: AsyncSession = Depends(db_helper.session_getter)):
    if not app.state.PARSED_JOBS_SESSION:
        for i in range(1, app.state.PARSED_PAGES + 1):
            ...
            datalist = hhru_parse_job(
                f"https://hh.ru/search/vacancy?text=&area=1&page={i}"
            )
...
async def listfilteredjobs(
    filters: schemas.Filters, db: AsyncSession = Depends(db_helper.session_getter)
):
    print(filters)
...
async def resums(db: AsyncSession = Depends(db_helper.session_getter)):
    if not app.state.PARSED_RESUMS_SESSION:
        for i in range(1, app.state.PARSED_PAGES + 1):
            datalist = hhru_parse_resum(
                f"https://hh.ru/search/resume?text=&area=1&items_on_page=100&page={i}"
            )
...
async def listfilteredresums(
    filters: schemas.FiltersResums, db: AsyncSession = Depends(db_helper.session_getter)
):
    print(filters)
